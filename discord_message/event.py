...
def after_setup(plugin_meta: PluginMeta, config: Dict[str, Any]):
    ...
    """
    discord.set_config(config.get('webhook'), config.get('proxy'))
...
@plugin.on_event(
    bind_event=[EventType.DownloadCompleted, EventType.SubMedia, EventType.DownloadStart, EventType.SiteError], order=1)
def on_event(ctx: PluginContext, event_type: str, data: Dict):
    """
    触发绑定的事件后调用此函数
    函数接收参数固定。第一个为插件上下文信息，第二个事件类型，第三个事件携带的数据
    """
    discord.send_by_event(event_type, data)
